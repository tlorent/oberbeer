...
class Brewery extends React.Component {
  ...
  render() {
    ...
    const {
      address, city, daysOpen, img, name, zipcode, beers
    } = this.props;
    return (
      <div className="brewery">
        <div className="brewery__profile">
          <img src={`${img}`} alt="Logo Brouwerij de Molen" className="brewery__logo" />
          <div className="brewery__profile-box">
            <p className="brewery__name">{`${name}`}</p>
            <p className="brewery__city">{`${city}`}</p>
          </div>
        </div>
        {isDetailVisible && (
          <div className="brewery__details-box">
            <p className="brewery__details-text">{`Meer informatie over ${name} 🍻`}</p>
          </div>
        )}
        <div
          className={`
        ...
        >
          ...
          "
          >
            <p className="brewery__address-title">Adres</p>
            <p className="brewery__address">{`${address}`}</p>
          </div>
          <div
            className="
              brewery__information-box
          ...
          "
          >
            <p className="brewery__postcode-title">Postcode</p>
            <p className="brewery__postcode">{`${zipcode}`}</p>
          </div>
          <div
            className="
              brewery__information-box
          ...
          "
          >
            <p className="brewery__opening-title">Open</p>
            {daysOpen.map((day, index) => (
              <p className="brewery__opening" key={index}>
        ...
        {beers.length > 0 && (
          <React.Fragment>
            ...
            >
              ...
              "
              >
                {beers.map(({ name: beerName, alcohol, style }, index) => (
                  <p className="brewery__beer" key={index}>
                    {`${beerName}: `}
                    <span className="brewery__beer-info">{`${style} (${alcohol}%)`}</span>
                  </p>
                ))}
              </div>
            </div>
        ...
        >
          <img alt="arrow pointing down" src="static/icons/arrow-down-2.svg" />
        </div>
      </div>
    );
  }
