...
import { getBeers } from '../../services/breweries-api';
import './search.scss';
import breweriesList from '../../firebase';
...
class Search extends React.Component {
  ...
  findBrewery = (zip, array) => {
    ...
    const match = array.reduce((accumulator, curr) => (Math.abs(curr - zip) < Math.abs(accumulator - zip) ? curr : accumulator));

    const { breweries, beers } = this.state;

    // first find the matching brewery
    const brewery = breweries.find((item) => item.zipcode.substring(0, 4) === match);
    ...
    return (
      <React.Fragment>
        <div className="search">
          ...
          <h3 className="search__subtitle">Vind een brouwerij bij jou in de buurt</h3>
          ...
          <p className="search__text">
            Na een dag hard werken heb je dat speciaal gebrouwen biertje wel verdiend! Maar geen
            idee naar welke brouwerij je wilt? Of sta je open voor een verrassing? Typ een postcode
            in en Oberbeer geeft je advies. Geniet van je bier!
          </p>
          {isBreweryVisible
